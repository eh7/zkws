...
#include "circom.hpp"
#include "calcwit.hpp"
void Inverse_0_create(uint soffset,uint coffset,Circom_CalcWit* ctx,std::string componentName,uint componentFather);
void Inverse_0_run(uint ctx_index,Circom_CalcWit* ctx);
...
ctx->componentMemory[coffset].templateName = "Inverse";
...
Fr_neq(&expaux[0],&signalValues[mySignalStart + 1],&circuitConstants[0]); // line circom 8
if(Fr_isTrue(&expaux[0])){
{
PFrElement aux_dest = &signalValues[mySignalStart + 2];
// load src
Fr_div(&expaux[0],&circuitConstants[1],&signalValues[mySignalStart + 1]); // line circom 8
// end load src
...
Inverse_0_create(1,0,ctx,"main",0);
Inverse_0_run(0,ctx);
}
